...
- **Установщик NSIS + перезагрузка Windows целиком.** Проверено то, что
  ...
  **2026-09-23 (пачка `23`, А0):** на `beta` с v0.0.84, поставленной
  установщиком, есть и значение `HKCU\…\Run\Lumepeer`, и маркер
  `%APPDATA%\io.insigmo.lumepeer\autostart-first-launch`. Перезагрузку не
  проверяли, она вынесена в `23`, задача Б4.
