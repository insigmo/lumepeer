...
export enum WireCodec {
  H264 = 0,
  Av1 = 1,
  // 2 was H.265, retired with the codec itself (ADR 0072) and deliberately
  // left unassigned rather than reused.
  Vp9 = 3,
}
...
export async function supportedOptionalCodecs(): Promise<WireCodec[]> {
  ...
  for (const codec of [WireCodec.Av1, WireCodec.Vp9] as const) {
    try {
      const support = await VideoDecoder.isConfigSupported({
