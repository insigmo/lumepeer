...
// sandbox, or the Windows Media Foundation COM calls the hardware H.264
// encoder needs (every `IMFTransform`/`IMFSample`/... call in the `windows`
// crate's MF bindings is `unsafe fn`). Exactly three modules opt back in,
// each with a `reason`: `decode::shm` (ADR 0005), `decode::windows_sandbox`
// (ADR 0007) and `encode::windows` (ADR 0011). Every block in all three
// carries a SAFETY note and is covered by tests, as §21 requires. Nothing
// else in the crate may opt in.
#![deny(unsafe_code)]
#![warn(missing_docs)]
