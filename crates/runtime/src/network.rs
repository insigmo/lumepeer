...
struct ViewFeed {
    ...
    /// wire byte (§11; ADR 0067). Starts at 0 (H.264) and is written only when
...
/// asked anything (§10; ADR 0044, ADR 0084 §6, ADR 0106).
///
/// A free function rather than a line inside `may_return_without_asking` so
/// the rule it encodes can be checked on its own: both halves, and neither
/// enough alone. `trusted` is a decision somebody made about this host and
/// nothing else sets it; `remembered` is whether the return could actually be
/// silent, because there is nothing to answer a challenge with otherwise.
///
/// ADR 0106 narrowed what this decides. It used to gate the *dialing* — a
/// host that failed it was left with the "connect again" button. Now every
/// host whose session this node was in is dialed again; this is what says
/// whether that return may happen without a person at either end pressing
/// anything, which is what "reconnect on its own" promises in the address
/// book and all it has ever promised.
...
enum ActorCommand {
    ...
    /// Guest side: which optional codecs this process's `WebView` can decode,
    /// as [`MediaCodec`] wire bytes (§11; ADR 0070). Reported once by the main
    /// window at startup, before any dial.
    DecoderCodecs {
        codecs: Vec<u8>,
        reply: oneshot::Sender<()>,
    },
...
impl ActorHandle {
    ...
    /// Records which optional codecs this process's `WebView` can decode, so
    /// this node's next `Hello` can advertise them (§11; ADR 0070).
    ///
    /// `codecs` are `MediaCodec` wire bytes, exactly as
    /// `view-decoder.ts`'s `supportedOptionalCodecs()` answered. This carries
    /// no authorization of its own: the worst a wrong answer can do is make
    /// this guest ask for a codec it cannot decode, and it is the only side
    /// that pays for that (§2.3).
    ///
    /// # Errors
    /// [`ActorError::ChannelClosed`] if the actor task is gone.
    pub async fn report_decoder_codecs(&self, codecs: Vec<u8>) -> Result<(), ActorError> {
        let (reply, rx) = oneshot::channel();
        self.tx
            .send(ActorCommand::DecoderCodecs { codecs, reply })
            .await
            .map_err(|_| ActorError::ChannelClosed)?;
        rx.await.map_err(|_| ActorError::ChannelClosed)
    }
...
/// Which optional codecs (beyond the mandatory H.264 baseline) a guest can
/// decode (§11; ADR 0067, ADR 0070).
///
/// One small set read in two directions rather than a pair of types.
/// On a **host** it is what a peer's `Hello` advertised, and it decides what
/// `MessageKind::MediaCodec` may name. On a **guest** it is what this
/// process's own `WebView` answered and it decides what this node's `Hello`
/// advertises in the first place. Unlike
/// [`ReceiverReports`]/[`StreamScaleFeature`] the two halves are not a
/// per-peer pair: only a host ever chooses and announces a codec, and a
/// guest's own decoding ability is one fact about the process, not one per
/// connection.
#[derive(Debug, Clone, Copy, Default)]
struct GuestCodecSupport {
    /// AV1: [`FEATURE_CODEC_AV1`].
    av1: bool,
    /// VP9: [`FEATURE_CODEC_VP9`].
    vp9: bool,
}
...
impl GuestCodecSupport {
    ...
    /// Guest side: the answer `view-decoder.ts`'s `supportedOptionalCodecs()`
    /// gave, as [`MediaCodec`] wire bytes (§11; ADR 0070).
    ///
    /// A byte this build has no name for is dropped rather than refused:
    /// it means a webview bundle newer than this binary, and the honest
    /// answer from a binary that has never heard of that codec is "no" — it
    /// could not name it in a `Hello` even if it wanted to. `MediaCodec::H264`
    /// is ignored for the same reason it has no feature string: every peer
    /// decodes it and there is nothing to advertise.
    fn from_wire_bytes(codecs: &[u8]) -> Self {
        let advertised = |codec: MediaCodec| codecs.contains(&codec.to_wire());
        Self {
            av1: advertised(MediaCodec::Av1),
            vp9: advertised(MediaCodec::Vp9),
        }
    }

    /// Guest side: the `Hello.features` strings this set advertises, in the
    /// wire order of the codecs they name (§9.1; ADR 0070).
    fn features(self) -> Vec<String> {
        let mut features = Vec::new();
        if self.av1 {
            features.push(FEATURE_CODEC_AV1.to_owned());
        }
        if self.vp9 {
            features.push(FEATURE_CODEC_VP9.to_owned());
        }
        features
    }
...
/// `MessageKind::MediaCodec` this host is about to send (§11; ADR 0067).
///
...
struct ViewState {
    ...
    /// 0067). Written when a `MessageKind::MediaCodec` arrives; starts at 0
...
struct Actor {
    ...
    /// understanding of (§11; ADR 0067).
    guest_codec_support: std::collections::HashMap<NodeId, GuestCodecSupport>,
    /// Guest side: which optional codecs this process's own `WebView` said it
    /// can decode, for this node's `Hello` to advertise (§11; ADR 0070).
    ///
    /// Empty until the main window reports it, which it does once at startup.
    /// A dial that happens before that report — or in a build with no window
    /// at all, as the actor tests run — advertises nothing and gets H.264,
    /// which is the same place an older guest lands and is always correct.
    own_codec_support: GuestCodecSupport,
...
impl Actor {
    ...
    fn on_media_accepted(&mut self, connection: PeerConnection, peer: NodeId) {
        ...
        // §11, ADR 0067: the intersection of what this guest can decode with
    ...
    fn on_inbound(&mut self, peer: NodeId, id: u64, kind: &MessageKind) {
        ...
        match *kind {
            ...
            // about to carry (§11; ADR 0067). The byte already passed
    ...
    fn on_closed(&mut self, peer: NodeId, id: u64) {
        ...
        // with ADR 0084's wait for a new session behind it. Read here rather
        // than below because it is also what decides whether the window
        // closes (ADR 0105).
        let resume = closed
            .as_ref()
            .filter(|handle| handle.peer_minor >= SESSION_RESUME_MINOR)
            .map(|handle| handle.session_id);
        // Every session this node was actually in is waited for, at every
        // host rather than only a trusted one with a remembered password
        // (docs/bugs/22 task 3; ADR 0106). What the wait produces at an
        // untrusted host is an ordinary connect that asks for consent again —
        // worth trying for as long as ADR 0084 allows, rather than a button
        // twelve seconds after the link stuttered.
        //
        ...
        // vanishing and reappearing (docs/bugs/22 task 2; ADR 0105). Either
        // branch writes the history row this wait reads its code out of, so
        // the row exists by the time anything below looks for it.
        if was_watching {
            self.park_view(peer);
        } else {
            self.stop_view(peer);
    ...
    fn handle_command(&mut self, command: ActorCommand) {
        match command {
            ...
            ActorCommand::DecoderCodecs { codecs, reply } => {
                self.own_codec_support = GuestCodecSupport::from_wire_bytes(&codecs);
                tracing::info!(
                    codecs = ?self.own_codec_support,
                    "this webview reported what it can decode (§11; ADR 0070)"
                );
                let _ = reply.send(());
            }
            ...
            ActorCommand::HistorySetTrusted {
                ...
                // Withdrawing this used to stop a wait running on that very
                // host, because the flag was what allowed the wait at all.
                // Since ADR 0106 it is not: every host whose session this
                // node was in is waited for, and this flag only says whether
                // the return may be silent. It is re-read at the challenge,
                // so moving it mid-wait takes effect on the next one without
                // taking the wait — or the window it is holding up (ADR 0105)
                // — away from somebody who never asked for that.
                let _ = reply.send(self.history.set_trusted(&label, trusted));
            }
    ...
    /// Guest side: whether a session with `host_tag` may be raised again with
    /// nobody at either end asked anything (§10; ADR 0044, ADR 0084 §6,
    /// ADR 0106).
    ///
    /// Both halves, and neither is enough on its own. The user must have
    /// marked the host as one this node may come back to on its own — nothing
    /// about connecting, being granted a role or saving a password sets that
    /// flag — **and** this node must remember a device password for it,
    /// because a silent return is exactly a challenge answered without a
    /// prompt.
    ///
    /// Read only where a challenge is about to be answered by itself. What it
    /// no longer decides is whether to dial at all: ADR 0106 keeps dialing a
    /// host whose session this node was in either way, and a host that fails
    /// this one simply gets asked, by the person at whichever end the host
    /// chooses to ask.
    fn may_return_without_asking(&self, host_tag: &str) -> bool {
    ...
    /// ADR 0089).
    ///
    /// The session is gone on the host, so claiming it again is pointless —
    /// but the host is plainly there, because it just answered. What is left
    /// is ADR 0084's wait for a *new* session, which ADR 0106 runs for every
    ...
    /// 0084's wait for a new session: the window is over and the host never
    /// answered, so what is asked for next is a session rather than the
    /// session (ADR 0106).
    ...
    fn on_dial_failed(&mut self, peer: NodeId, tag: &str, error: &NetError, resuming: bool) {
        // A host that answered has answered, even when its answer arrived as
        // a failed dial rather than as a message: an invalid or expired
        // ticket, a protocol version this node cannot speak, a host that
        // cannot queue the request at all. None of those changes by being
        // asked again in fifteen seconds, and ADR 0106 dials often enough
    ...
    fn on_reconnect_wait_tick(&mut self, generation: u64) {
        ...
        // A host that answered has answered (ADR 0100). This wait exists
        // because nobody said anything; a refused consent, a refused device
        // password and a host that cannot decide at all are all somebody
        ...
        // The trusted flag used to be re-read here, and a host that lost it
        // mid-wait ended the wait. ADR 0106 took that away: the flag no
        // longer decides whether this node dials, only whether the return may
        // be silent, and that is re-read where a challenge is answered. What
...
pub fn spawn_actor_with(
    ...
    let actor = Actor {
        ...
        own_codec_support: GuestCodecSupport::default(),
...
mod tests {
    ...
    /// defines and nothing else, and any one of them alone is enough to prove
    /// this guest understands the message.
    #[test]
    ...
    /// AV1 is chosen only with mutual hardware support (§11; ADR 0069): a
    /// guest claiming AV1 gets it exactly when this machine's own probe says
    /// there is a hardware AV1 encoder, and gets the H.264 fallback
    /// otherwise. Written against the probe's live answer rather than against
    /// a constant so it runs identically on a machine with AV1 hardware and
    /// on one without — the claim is that the two agree, not that either is
    /// true here.
    #[test]
    fn av1_is_chosen_exactly_when_the_hardware_probe_says_so() {
    ...
    /// to, and the strings it sends are read back by the host half of the
    /// same type. A byte no `MediaCodec` names — a webview bundle newer than
    /// this binary — is dropped rather than turned into a feature string this
    /// build cannot name.
    #[test]
    fn the_codecs_a_webview_reports_are_the_strings_a_hello_advertises() {
        let reported = GuestCodecSupport::from_wire_bytes(&[
            MediaCodec::Av1.to_wire(),
            MediaCodec::Vp9.to_wire(),
            200,
        ]);
        assert_eq!(
            reported.features(),
            vec![FEATURE_CODEC_AV1.to_owned(), FEATURE_CODEC_VP9.to_owned()]
        );
        let seen_by_the_host = GuestCodecSupport::from_features(&reported.features());
        assert!(seen_by_the_host.av1);
        assert!(seen_by_the_host.vp9);
    }

    /// H.264 is the baseline every peer decodes and has no feature string, so
    /// a webview that reports it and nothing else advertises nothing at all —
    /// and a host therefore never sends it `MediaCodec` (ADR 0067).
    #[test]
    fn reporting_only_h264_advertises_nothing_and_negotiates_nothing() {
        let reported = GuestCodecSupport::from_wire_bytes(&[MediaCodec::H264.to_wire()]);
        assert!(reported.features().is_empty());
    ...
    /// choose from, so no advertisement a guest can make — including one
    /// naming the retired H.265 wire byte — reaches anything else.
    #[test]
    fn nothing_a_guest_advertises_negotiates_a_codec_beyond_av1_and_the_baseline() {
        // 2 was H.265's wire byte before it was removed; a peer still
        // sending it names a codec this build has no `MediaCodec` for.
    ...
    /// ADR 0084 §6 as ADR 0106 left it, and the half of the story that is
    /// about *not* acting: a machine this node happens to know the password
    /// for is not thereby a machine it may let itself back into silently, and
    /// a machine somebody marked but whose password is not remembered has
    /// nothing to answer the challenge with.
    ///
    /// Stated as a truth table because that is what the rule is. What failing
    /// it costs is no longer the wait itself — ADR 0106 dials either way —
    /// but the silence: the challenge arrives and somebody answers it.
    #[test]
    fn returning_without_asking_needs_both_halves() {
        assert!(auto_reconnect_allowed(true, true));
        assert!(
            !auto_reconnect_allowed(false, true),
            "a remembered password was treated as permission to return unasked"
        );
        assert!(
            !auto_reconnect_allowed(true, false),
            "a marked host with no remembered password has nothing to answer with"
        );
    ...
    async fn a_refused_resume_stops_resuming(refusal: ResumeRefusal) {
        ...
        // The resume is over — that is what this test is named for — but the
        // host plainly exists, because it just answered. ADR 0106: what
        // follows is ADR 0084's wait for a *new* session, at this host as at
        // any other, rather than a button twelve seconds after the link
        // stuttered.
        wait_for_phase(&guest, ConnectPhase::WaitingForHost).await;
        let state = guest.connect_state().await.unwrap();
        assert_eq!(
            state.code, None,
            "a refused resume is not a failure the user has to answer"
        );
        assert_ne!(
            state.phase,
            ConnectPhase::Resuming,
            "the refused claim must not be made again"
        );
    }
